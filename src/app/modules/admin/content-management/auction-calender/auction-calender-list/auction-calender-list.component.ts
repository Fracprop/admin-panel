    ...
    styles: [
        `
            .auction-grid {
                ...
                @screen md {
                    grid-template-columns: 50px 200px 200px 200px 200px 300px;
                }
...
export class AuctionCalenderListComponent implements OnInit {
    ...
    deleteContent(userId: string) {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                this._auctionService.deleteAuction(userId).subscribe(
                    (response) => {
                        this.getListing();
                    },
                    (err) => {
    ...
    view(id: any, isApprove) {
        const dialogRef = this._matDialog.open(ViewBidsDialogComponent, {
            panelClass: 'custom-dialog-container',
            disableClose: true,
            data: {
                type: 2,
